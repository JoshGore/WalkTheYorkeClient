...
const MenuContainer: React.FunctionComponent<MenuProps> = ({
  ...
  const [menuMode, setMenuMode] = useState<MenuModes>(
    window.innerHeight > window.innerWidth ? 'bottom' : 'side',
  );
    ...
    setContentTopRight({
      top:
        menuState === 'collapsed'
          ...
          : menuState === 'visible'
          ? contentTopVisible()
          : contentTopFullscreen,
      right:
        menuState === 'fullscreen'
          ? contentRightFullscreen
          : contentRightVisible(),
    });
  });
  ...
  useEffect(() => {
    setPlaceholderHeight({
      height:
        menuState === 'collapsed'
          ? placeholderHeightCollapsed()
          : placeholderHeightVisible(),
    });
  });
  useEffect(() => {
  ...
  return (
    <Grid
      ...
      className={`${classes.root} ${
        menuMode === 'bottom' ? classes.portrait : classes.landscape
      }`}
    >
      <animated.div
        className={`${classes.placeholder} ${
          menuMode === 'bottom'
            ? classes.placeholderBottom
            : classes.placeholderSide
        } `}
        style={{
      ...
      >
        ...
        <Scrollbars style={{ flex: '1 1 auto' }} onScroll={handleBodyScroll}>
          {body}
        </Scrollbars>
      </animated.div>
      <div
